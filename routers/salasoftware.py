...
@router.post("/salasoft", status_code=status.HTTP_201_CREATED, response_model=schemas.SalaSoftware)
def CreatesalaSoftware(salasoftware: schemas.CreatesalaSoftware, db: Session = Depends(get_db)):

  new_salaSoftware  = models.SalaSoftware(**salasoftware.dict())
  db.add(new_salaSoftware)
  db.commit()
  db.refresh(new_salaSoftware)

  return new_salaSoftware
